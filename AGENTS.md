...
| `src/engine/` | orchestration: `ports.ts` (side-effect contracts), `fleet.ts` (consumer data schemas), `query.ts` (`--on`), `waves.ts` (waves, current zone), `nix-output.ts` (eval, build log and flake lock parsers), `flake-sources.ts` (locked sources a builder fetches itself), `recorder.ts` (stream → `var/deployments/`), `hosts.ts` (host table, transition guard), `presence.ts` (pings), `decisions.ts` (failed and lost hosts), `known-errors.ts` (signature → plain message), `lock.ts` (busy lock: holder, takeover, resume offer), `deploy.ts` (one host: copy, activation), `settle.ts` (reconnection: result file, rollback timer), `rollback.ts` (forced rollback of the fleet or of one host), `pool.ts`, `report.ts` (report and incidents message), `matrix.ts` (alert rooms, message texts), `run.ts` (orchestration: lock, steps, exit code) | model, ai (analysis hand-off) |
...
| `src/adapters/` | real ports: `process.ts` (`CommandRunner`, one process group per command), `clock.ts`, `lock.ts` (flock(2) through `bun:ffi`), `store.ts` (`var/deployments/`), `sources.ts` (`SourceFiles`: trees the AI may read, confinement checked on the **resolved** path), `tail.ts` (bounded head and tail reads), `mcp.ts` (`ToolServer`: loopback endpoint, ephemeral port, bearer token), `localhost.ts`, `channel.ts` (`EventChannel` within the process) | model, ports |
| `src/ai/` | AI tools as executables, no SDK and no API: `model.ts` (`--ai-model` target), `providers.ts` (argv of `claude` and `opencode`, prompt on stdin, every built-in tool denied), `session.ts` (one question, one streamed answer), `paths.ts` (trees the AI may read), `context.ts` (`ToolContext`: the narrow face the tools get), `tools/` (one file per family, `registry.ts` the only list), `rpc.ts` (MCP over JSON-RPC, pure dispatch), `server.ts` (endpoint of the run, opened once), `prompts.ts` (built from the fold, never from the engine), `analysis.ts` (triggers, and what the report keeps), `repair.ts` (the session allowed to act) | model, engine |
...
| Programs | package `PATH` suffix: `git`, `nix-eval-jobs`, `ssh`; host: `nix`; unit: `just`, `statix`, `deadnix`, `nixfmt`, `treefmt`, `dnf-generator`, `cargo`, `curl` and `jq` (`just send-msg`); `/run/wrappers/bin`: `sudo`, `ping` | argv names resolved from `PATH`, never a store path |
| AI context | `aiContext` → `/etc/fleet-update/ai-context`, written whenever set, `enable` or not (codev runs from sources) | `AI_CONTEXT_FILE` read by every non-resumed run: option > file > none |
| AI tools | never a package dependency (`claude-code` is unfree): module option `aiPackages` puts them on the unit's `PATH` | `claude` and `opencode` resolved from `PATH`; absent → one warning, run unaffected |
...
| Alert bot | `just send-msg <warnings\|incidents>`, body on stdin: rooms, token and homeserver stay in the framework; exit `10` not configured, `11` refused; markdown rendered to `formatted_body` | `--send-report` builds the text and pipes it, one call per room; a `10` stops the remaining rooms, any failure is exit `3`; markdown subset: headings, bullets, bold, code, links |
