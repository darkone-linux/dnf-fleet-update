...
| `src/adapters/` | real ports: `process.ts` (`CommandRunner`, one process group per command), `clock.ts`, `lock.ts` (flock(2) through `bun:ffi`), `store.ts` (`var/deployments/`), `localhost.ts`, `channel.ts` (`EventChannel` within the process), `matrix.ts` (client API, the only network call) | model, ports |
...
| Programs | package `PATH` suffix: `git`, `nix-eval-jobs`, `ssh`; host: `nix`; unit: `just`, `statix`, `deadnix`, `nixfmt`, `treefmt`, `dnf-generator`, `cargo`, `sops` (`--send-report`); `/run/wrappers/bin`: `sudo`, `ping` | argv names resolved from `PATH`, never a store path |
...
| Alert bot | `just configure-alert-bot`: bot and room ids in `var/generated/matrix.nix` (not merged into `network.nix`), token `alertmanager-matrix-token` in `usr/secrets/secrets.yaml` | `--send-report` reads that file, `sops -d --extract` for the token, one `m.text` PUT per room on `https://matrix.<domain>`, neutral User-Agent |
