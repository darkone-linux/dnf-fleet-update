...
| `src/engine/` | orchestration: `ports.ts` (side-effect contracts), `fleet.ts` (consumer data schemas), `query.ts` (`--on`), `waves.ts` (waves, current zone), `nix-output.ts` (eval and build log parsers), `recorder.ts` (stream → `var/deployments/`), `hosts.ts` (host table, transition guard), `presence.ts` (pings), `decisions.ts` (failed and lost hosts), `deploy.ts` (one host: copy, activation), `settle.ts` (reconnection: result file, rollback timer), `rollback.ts` (forced rollback of the fleet or of one host), `pool.ts`, `report.ts`, `run.ts` (orchestration: lock, steps, exit code) | model |
