...
| `src/engine/` | orchestration: `ports.ts` (side-effect contracts), `fleet.ts` (consumer data schemas), `query.ts` (`--on`), `waves.ts` (waves, current zone), `nix-output.ts` (eval and build log parsers), `recorder.ts` (stream → `var/deployments/`), `hosts.ts` (host table, transition guard), `presence.ts` (pings), `decisions.ts` (failed and lost hosts), `known-errors.ts` (signature → plain message), `lock.ts` (busy lock: holder, takeover, resume offer), `deploy.ts` (one host: copy, activation), `settle.ts` (reconnection: result file, rollback timer), `rollback.ts` (forced rollback of the fleet or of one host), `pool.ts`, `report.ts` (report and incidents message), `matrix.ts` (alert rooms, message texts), `run.ts` (orchestration: lock, steps, exit code) | model, ai (analysis hand-off) |
