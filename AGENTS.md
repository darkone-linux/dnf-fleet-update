...
| `src/engine/steps/` | one module per step (`update.ts`, `select.ts`), receives `RunContext` (`context.ts`: ports, parameters, `flow.ts` early ends, questions); `exec.ts` runs their commands | model, engine |
