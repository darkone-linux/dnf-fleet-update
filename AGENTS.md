...
| Alert bot | `just send-msg <warnings\|incidents>`, body on stdin: rooms, token and homeserver stay in the framework; exit `10` not configured, `11` refused | `--send-report` builds the text and pipes it, one call per room; a `10` stops the remaining rooms, any failure is exit `3` |
